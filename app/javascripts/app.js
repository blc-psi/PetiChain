...
window.createPetition = function createPetition() {
  ...
  c6.innerHTML = '<button type="button" class="btn btn-primary btn-sm" onclick="confirmPetition(' + row_index + ')">Accept</button> <button type="button" class="btn btn-primary btn-sm" onclick="rejectPetition(' + row_index + ')">Reject</button>';
}
...
window.confirmPetition = function confirmPetition(index) {
  ...
  var id = row.cells[0];
  var titel = row.cells[1];
  var description = row.cells[2];
  var startTime = row.cells[3];
  var endTime = row.cells[4];
