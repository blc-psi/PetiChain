...
var petregAddress = '0x4C4db10AecB6F2751A6DfaE63ead5839B30a1CcA';
...
var petreg = petregContract.at(petregAddress);
...
window.listPetitions = function listPetitions() {
  ...
  while (i < table.rows.length) {
    ...
    pet = petCon.at(petreg.hasAddress(id));
  ...
  while (listCount < parseInt(petreg.count())) {
    pet = petCon.at(petreg.hasAddress(listCount));
...
window.vote = function vote(id, choice) {
  ...
  while (i < num) {
    ...
    pet.vote(choice, {from: web3.eth.defaultAccount});
    i += 1;
  }
...
window.confirmPetition = function confirmPetition(index) {
  ...
  if (web3.eth.defaultAccount.toLowerCase() != authAddr.toLowerCase()) {
    document.getElementById('admin-error-out').innerText = "You're not allowed to accept or reject pending Petitions";
    return;
  }
